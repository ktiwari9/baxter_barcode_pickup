...
int global_squareness_ratio=70;
cv::Mat greenFilt;
...
public:
  ...
  void cubelet_VS(const sensor_msgs::ImageConstPtr& msg)  {
	...
	// cv_ptr = pointer to our image
	// cv_ptr->image the image within the pointer
	
	inRange(cv_ptr->image,cv::Scalar(0, 205, 0),cv::Scalar(255,255,255),greenFilt);
	cv::namedWindow("Green Image");
    cv::imshow("Green Image",greenFilt);
    
    cv::Mat filtered_image = greenFilt;
    
    CvMoments gMoments;//make moments object
    gMoments = moments(greenFilt);//get the moments
    double moment10 = cvGetSpatialMoment(&gMoments, 1, 0);//sum x moments
    double moment01 = cvGetSpatialMoment(&gMoments, 0, 1);//sum y moments
    double area = cvGetCentralMoment(&gMoments, 0, 0);//get moment area
    int posX;
	int posY;
	posX = moment10/area;//parse out x center
	posY = moment01/area;//and y center
	printf("position (%d,%d)\n", posX, posY);
		std::ofstream outfile;
	outfile.open("cube_vision.txt", std::ios_base::app);
	outfile << posX << "," << posY << "\n";
	outfile.close();
...
*/
    ...
    printf("P1:[%f,%f], P2:[%f,%f], P3:[%f,%f], P4:[%f,%f] \n",new_rect_points[0].x,
		new_rect_points[0].y,new_rect_points[1].x,new_rect_points[1].y,new_rect_points[2].x,
		new_rect_points[2].y,new_rect_points[3].x,new_rect_points[3].y);
	*/
		
	//double midX = (new_rect_points[0].x + new_rect_points[1].x + new_rect_points[2].x + new_rect_points[3].x)/4;
	//double midY = (new_rect_points[0].y + new_rect_points[1].y + new_rect_points[2].y + new_rect_points[3].y)/4;
